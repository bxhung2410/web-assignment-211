
    <div class="page-wrapper">
    <?php 
      require_once "./mvc/views/".$data["header"].".php";
    ?>
        <main class="page-main">
            <div class="section-first-screen">
                <div class="first-screen__bg" style="background-image: url(assets/img/pages/contacts/bg1.jpg)"></div>
                <div class="first-screen__content">
                    <div class="uk-container">
                        <nav class="category-nav">
                            <nav class="uk-navbar-container uk-navbar-transparent" data-uk-navbar>
                                <ul class="uk-navbar-nav">
                                    <li>
                                        <a href="../Home/catalog">
                                            <div class="nav-item"><span class="nav-item__icon"><img src="assets/img/icons/menu/face.png" alt="face care" data-uk-svg></span><span class="nav-item__text">chăm sóc da</span></div>
                                        </a>
                                        <li>
                                            <a href="../Home/catalog">
                                                <div class="nav-item"><span class="nav-item__icon"><img src="assets/img/icons/menu/hair.png" alt="hair care" data-uk-svg></span><span class="nav-item__text">chăm sóc tóc</span></div>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="../Home/catalog">
                                                <div class="nav-item"><span class="nav-item__icon"><img src="assets/img/icons/menu/lips.png" alt="lips care" data-uk-svg></span><span class="nav-item__text">chăm sóc môi</span></div>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="../Home/catalog">
                                                <div class="nav-item"><span class="nav-item__icon"><img src="assets/img/icons/menu/body.png" alt="body care" data-uk-svg></span><span class="nav-item__text">chăm sóc cơ thể</span></div>
                                            </a>
                                        </li>
                                </ul>
                            </nav>
                        </nav>
                        <div class="first-screen__box">
                            <h2 class="first-screen__title">Original VietNam</h2>
                            <div class="first-screen__breadcrumb">
                                <ul class="uk-breadcrumb">
                                    <li><a href="../Home/index">Home</a></li>
                                    <li> <a href="../News/index">News Blog</a></li>
                                    <li> <span>Full post</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="page-content">
                <div class="uk-margin-large-top uk-container uk-container-small">
                    <article class="article-full">
                        <div class="article-full__info">
                            <div class="article-full__author"><i class="fas fa-user"></i><span>By Cloudy Thy</span></div>
                            <div class="article-full__category"><i class="fas fa-folder-open"></i><span>Posted in Event</span></div>
                            <div class="article-full__date"><i class="fas fa-calendar-alt"></i><span>Jan 22, 2021</span></div>
                            <div class="article-full__comments"><i class="fas fa-comment"></i><span>10</span></div>
                        </div>
                        <div class="article-full__image">
                            <a href="../News/blog/2"><img src="assets/img/blog/blog2_anh1.jpg" alt="img-article"></a>
                        </div>
                        <div class="article-full__content">
                            <h1>Hương tan muộn phiền, đón tết bình an</h1>
                            <p style="font-size: 16.5px; font-weight: 500; font-style: italic;">Cầu chúc những điều may mắn và tốt đẹp nhất sẽ đến với bạn trong năm mới Tân Sửu 2021.</p>
                            <p>Xua tan những phiền muộn, giải tỏa những căng thẳng bằng mùi hương là một liệu pháp thư giãn tinh thần, một món quà chào năm Tân sửu 2021 mà Cocoon đã ấp ủ từ rất lâu, nay mới có cơ hội chia sẻ cùng các bạn.</p>
                            <p>Liệu pháp mùi hương trong bộ sưu tập giúp "xả stress” với tên gọi HƯƠNG TAN MUỘN PHIỀN lần này của chúng tôi vẫn sẽ đến từ những loài cỏ cây, hoa lá quen thuộc, mà chắc có lẽ bạn đã bắt gặp đâu đó trong những cuộc du ngoạn
                                về với tự nhiên của mình. Đó là nốt hương nồng ấm từ khuynh diệp, nốt the lạnh của bạc hà và nốt ngọt thanh đầy hoang dại của hoa phong lữ. Một sự kết hợp hoàn hảo dành cho những ai đang mỏi mệt, đang kiếm tìm sự an yên
                                từ trong tâm hồn. Và điều tuyệt vời là bạn có thể cảm nhận, có thể tận hưởng và chìm đắm trong “bản giao thoa” của những nốt hương ngay từ những ve vuốt nhẹ nhàng trên làn da cơ thể, ngay tại căn phòng tắm thân quen của
                                mình, và điều đấy sẽ giúp bạn yêu việc tắm gội hơn bao giờ hết.</p>
                            <div><img class="uk-width-1-1" src="assets/img/blog/blog2_anh2.jpg" alt="article"></div>

                            <p style="font-size: 16.5;">Bộ sưu tập "HƯƠNG TAN MUỘN PHIỀN" sẽ mở đầu bằng 2 sản phẩm:</p>
                            <h3>1. Gel tắm khuynh diệp bạc hà</h3>
                            <ul>
                                <li>Dung tích: 500ml</li>
                                <li>Giá sản phẩm: 165.000 đồng</li>
                                <li>Công dụng chính: Làm sạch da toàn thân, mang lại làn da mềm mịn, tươi mới. Liệu pháp mùi hương giúp thư giãn, tạo cảm giác nhẹ nhàng giúp xả stress, mang lại tinh thần thông suốt</li>
                            </ul>

                            <h3>2. Sữa dưỡng thể khuynh diệp bạc hà</h3>
                            <ul>
                                <li>Dung tích: 140ml</li>
                                <li>Giá sản phẩm: 95.000 đồng</li>
                                <li>Công dụng:
                                    <ul style="list-style: '- ';">
                                        <li>Với kết cấu dạng sữa có khả năng thẩm thấu nhanh và không gây nhờn rít, sữa dưỡng thể giúp cung cấp độ ẩm cần thiết, mang lại làn da mềm mại và mịn màng.</li>
                                        <li>Liệu pháp mùi hương giúp thư giãn, tạo cảm giác nhẹ nhàng giúp xả stress, mang lại tinh thần thông suốt.</li>
                                    </ul>
                                </li>
                            </ul>



                            <div class="article-full__bottom">
                                <div class="article-full__tags">
                                    <div> <i class="fas fa-tags"></i><strong>Related Tags</strong></div><span>Skincare, BodyCare, Event</span>
                                </div>
                                <div class="article-full__share">
                                    <div> <strong>Share This</strong><i class="fas fa-share-alt"></i></div>
                                    <ul>
                                        <li><a href="#!"><i class="fab fa-twitter"></i></a></li>
                                        <li><a href="#!"><i class="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#!"><i class="fab fa-google-plus-g"></i></a></li>
                                        <li><a href="#!"><i class="fab fa-pinterest-p"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                            <div class="article-full__nav">
                                <ul class="articles-nav-list">
                                    <li class="articles-nav-item">
                                        <a class="articles-nav-item__link" href="../News/blog/2">
                                            <div class="articles-nav-item__img"><img src="assets/img/blog/mid_post.jpg" alt="article-thumb"></div>
                                            <div class="articles-nav-item__info">
                                                <div class="articles-nav-item__title">Cocoon - Tiên phong xu hướng mỹ phẩm thuần chay tại Việt Nam</div>
                                                <div class="articles-nav-item__user"> <i class="fas fa-user"></i><span>By Cloudy Thy</span></div>
                                            </div>
                                        </a>
                                        <div class="articles-nav-item__label">Prev Post</div>
                                    </li>
                                    <li class="articles-nav-item">
                                        <a class="articles-nav-item__link" href="../News/blog/3">
                                            <div class="articles-nav-item__img"><img src="assets/img/blog/next_post.jpg" alt="article-thumb"></div>
                                            <div class="articles-nav-item__info">
                                                <div class="articles-nav-item__title">“Festive Gift From Dak Lak” - Quà tặng Giáng Sinh từ Đắk Lắk</div>
                                                <div class="articles-nav-item__user"> <i class="fas fa-user"></i><span>By Cloudy Thy</span></div>
                                            </div>
                                        </a>
                                        <div class="articles-nav-item__label">Next Post</div>
                                    </li>
                                </ul>
                            </div>
                            <div class="section-article-reviews">
                                <!-- <div class="section-title">
                                    <div class="uk-h2">Bình luận (3)</div>
                                </div>
                                <div class="section-content">
                                    <ul class="uk-comment-list">
                                        <li>
                                            <article class="uk-comment">
                                                <header class="uk-comment-header">
                                                    <div class="uk-grid-small uk-grid-divider" data-uk-grid>
                                                        <div class="uk-width-auto@s"><img class="uk-comment-avatar" src="./assets/img/blog/img-reviews-1.png" alt></div>
                                                        <div class="uk-width-expand@s">
                                                            <div class="uk-flex uk-flex-middle uk-margin-small-bottom">
                                                                <h4 class="uk-comment-title uk-margin-remove">Huy Khoa</h4><span class="uk-text-meta uk-margin-small-left">Jan 25, 2021</span>
                                                            </div>
                                                            <div class="uk-comment-body">
                                                                <p>Nghe hay đấy, tôi sẽ mua thử.</p><a class="link-more" href="#!"><span data-uk-icon="arrow-right">Reply</span></a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </header>
                                            </article>
                                            <ul>
                                                <li>
                                                    <article class="uk-comment">
                                                        <header class="uk-comment-header">
                                                            <div class="uk-grid-small uk-grid-divider" data-uk-grid>
                                                                <div class="uk-width-auto@s"><img class="uk-comment-avatar" src="./assets/img/blog/img-reviews-2.png" alt></div>
                                                                <div class="uk-width-expand@s">
                                                                    <div class="uk-flex uk-flex-middle uk-margin-small-bottom">
                                                                        <h4 class="uk-comment-title uk-margin-remove">Đông Phong</h4><span class="uk-text-meta uk-margin-small-left">Jan 25, 2021</span>
                                                                    </div>
                                                                    <div class="uk-comment-body">
                                                                        <p>Tôi rất thích việc sản phẩm dùng nguyên liệu truyền thống, sẽ ủng hộ.</p><a class="link-more" href="#!"><span data-uk-icon="arrow-right">Reply</span></a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </header>
                                                    </article>
                                                </li>
                                            </ul>
                                        </li>
                                        <li>
                                            <article class="uk-comment">
                                                <header class="uk-comment-header">
                                                    <div class="uk-grid-small uk-grid-divider" data-uk-grid>
                                                        <div class="uk-width-auto@s"><img class="uk-comment-avatar" src="./assets/img/blog/img-reviews-3.png" alt></div>
                                                        <div class="uk-width-expand@s">
                                                            <div class="uk-flex uk-flex-middle uk-margin-small-bottom">
                                                                <h4 class="uk-comment-title uk-margin-remove">Thủy Ngọc</h4><span class="uk-text-meta uk-margin-small-left">Jan 25, 2021</span>
                                                            </div>
                                                            <div class="uk-comment-body">
                                                                <p>Bao bì thiết kế đơn giản mà đẹp mắt, rất hợp để mua làm quà tặng, tôi sẽ mua.</p><a class="link-more" href="#!"><span data-uk-icon="arrow-right">Reply</span></a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </header>
                                            </article>
                                        </li>
                                    </ul>
                                    <div class="block-form">
                                        <div class="section-title">
                                            <div class="uk-h2">Để lại bình luận</div>
                                        </div>
                                        <div class="section-content">
                                            <p>Địa chỉ email của bạn sẽ không được công khai, những ô bắt buộc sẽ có đánh dấu *</p>
                                            <form action="#!">
                                                <div class="uk-grid uk-grid-small uk-child-width-1-2@s" data-uk-grid>
                                                    <div class="uk-width-1-1"><textarea class="uk-textarea uk-form-large" placeholder="Message *"></textarea></div>
                                                    <div><input class="uk-input uk-form-large" type="text" placeholder="Your Name *"></div>
                                                    <div><input class="uk-input uk-form-large" type="text" placeholder="Email *"></div>
                                                    <div class="uk-width-1-1"><input class="uk-input uk-form-large" type="text" placeholder="Website"></div>
                                                    <div><button class="uk-button uk-button-large" type="submit">Đăng bình luận</button></div>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                </div> -->
                            </div>
                        </div>
                    </article>
                </div>
            </div>
        </main>
        <?php 
      require_once "./mvc/views/".$data["footer"].".php";
    ?>
    </div>
